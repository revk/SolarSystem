...
{				// Handle an event
  ...
  if (debug)
	...
	printf ("%02X", e->key);
      if (e->event == EVENT_FOB || e->event == EVENT_FOB_HELD || e->event == EVENT_FOB_ACCESS || e->event == EVENT_FOB_GONE || e->event == EVENT_FOB_NOACCESS)
	printf (" %s", e->fob);
      if (e->event == EVENT_RF)
   ...
   case EVENT_FOB_NOACCESS:
      {                         // Check users, doors?
         ...
         if (d >= 0)
         {
            ...
            dolog (mydoor[d].group_lock, "FOBACCESS", u ? u->name : NULL, port_name (port), "Access by fob %s%s", e->fob,
                   secure ? " (secure)" : "");
         else
