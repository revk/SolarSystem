...
void
mqtt_door (int d, const char *command, const unsigned char *afile)
{                               // Send door command
   unsigned int n;
...
static volatile port_p *
port_exit_set_n (volatile port_p * w, int n, int d)
{
   while (n--)
      if (w[n])
      {
...
static void *
keypad_login (keypad_t * k, user_t * u, const char *where, int secure)
{                               // PIN or fob login
   if (u && k->user != u)
   {
...
{                               // Update keypad display / beep (key non 0 for key press).
   ...
   if (alert)
      return keypad_message (k, "%s\n%04d-%02d-%02d %02d:%02d", alert, lnow.tm_year + 1900, lnow.tm_mon + 1, lnow.tm_mday,
...
{                               // Handle an event
   ...
   {                            // Debug logging
      ...
      if (e->event == EVENT_FOB || e->event == EVENT_FOB_HELD || e->event == EVENT_FOB_ACCESS || e->event == EVENT_FOB_GONE
          || e->event == EVENT_FOB_NOACCESS || e->event == EVENT_FOB_FAIL)
         printf (" %s", e->fob);
      if (e->event == EVENT_RF)
   ...
   case EVENT_INPUT:
      {
         ...
         if (app->isexit && e->state && app->door >= 0)
         {
            ...
            if (!door_locked (d))
            {
               ...
               {
                  dolog (mydoor[d].group_lock, "DOORAIRLOCK", NULL, doorno, "Airlock violation with DOOR%02d, exit rejected",
                         mydoor[d].airlock);
                  door_error (d, NULL);
   ...
   case EVENT_FOB_FAIL:
      {                         // Check users, doors?
         ...
         if (!secure || (u && e->message && !strncmp (u->afilecrc, e->message, 8)) || e->event == EVENT_FOB_HELD
         ...
         {                      // Time constraints
            ...
            if (expiry && expiry < now)
            {
               ...
               dolog (groups, e->event == EVENT_FOB_ACCESS ? "FOBBADACCESS" : "FOBEXPIRED", NULL, port_name (port),
         ...
         if (d >= 0)
         {
            ...
            {                   // disarm is the groups that can be disarmed by this user on this door.
               ...
               if (door[d].state == DOOR_NOTCLOSED || door[d].state == DOOR_OPEN || door[d].state == DOOR_PROPPED)
               {
                  ...
                  {
                     dolog (mydoor[d].group_lock, "DOORNOTPROPPED", u->name, doorno,
               ...
               } else
               {
                  if (u->group_open[secure] & mydoor[d].group_lock)
                  {
                     ...
                     {          // Allowed to be opened
                        ...
                        {       // Open it
                           dolog (mydoor[d].group_lock, "DOORUNLOCK", u->name, doorno, "Door open by fob %s%s", e->fob,
                                  secure ? " (secure)" : "");
            ...
            {                   // Held and we are allowed to set
               ...
               {
                  dolog (mydoor[d].group_lock, "FOBHELDIGNORED", u->name, doorno, "Ignored held fob %s%s as no setting options",
                         e->fob, secure ? " (secure)" : "");
                  door_error (d, afile);
               }
            } else
            {
               dolog (mydoor[d].group_lock, "FOBHELDIGNORED", u->name, doorno, "Ignored held fob %s%s as door cannot set alarm",
                      e->fob, secure ? " (secure)" : "");
               door_error (d, afile);
            }
         } else
         {
            if (e->event == EVENT_FOB_ACCESS)
               dolog (mydoor[d].group_lock, "FOBACCESS", u ? u->name : NULL, port_name (port), "Unknown door access by fob %s%s",
                      e->fob, secure ? " (secure)" : "");
            else
