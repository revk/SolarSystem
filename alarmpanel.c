...
port_exit_set_n (volatile port_p * w, int n)
{
   while (n--)
      if (w[n])
      {
         ...
         int pi = port_port (w[n]);
         if (pd < MAX_DEVICE && pi && pi <= MAX_INPUT)
            mydevice[pd].input[pi - 1].isexit = 1;
      }
   return w;
}
...
port_bell_set_n (volatile port_p * w, int n)
{
   while (n--)
   {
      ...
      int pi = port_port (w[n]);
      if (pd < MAX_DEVICE && pi && pi <= MAX_INPUT)
         mydevice[pd].input[pi - 1].isbell = 1;
   }
   return w;
}
         ...
         while (pl)
         {
            ...
            {                   // Resistance and response
               ...
               {                // resistance thresholds
                  unsigned int q = 0;
                  for (q = 0;
                       q < sizeof (rio_thresholds) / sizeof (*rio_thresholds) && strcasecmp (rio_thresholds[q].name, v); q++);
...
dologger (CURL * curl, log_t * l)
{
   ...
   if (!l->type || strcasecmp (l->type, "KEEPALIVE"))
      syslog (LOG_INFO, "%*s %s %s %s %s %s", MAX_GROUP, groups,
              l->type ? : "?", l->port ? : "", name ? : "", l->user ? : "", l->msg ? : "");
   if (debug)
         ...
         else if (state[STATE_ARM] & (1 << n))
         {
            for (s = 0;
                 s < STATE_TRIGGERS && (s == STATE_FAULT || s == STATE_WARNING
                                        || s == STATE_ZONE || s >= STATE_USER1 || !(state[s] & (1 << n))); s++);
   ...
   case EVENT_KEEPALIVE:
      {
         ...
         if (e->errors > 5 || e->stalled > 1 || !e->rx || !e->tx)
         {
            ...
            dolog (groups, "BUSERROR", NULL, busno,
                   "Bus reports errors:%d stalled:%d retries:%d tx:%d rx:%d", e->errors, e->stalled, e->retries, e->tx, e->rx);
         } else
         {
            if (mybus[n].fault)
            {
   ...
   case EVENT_INPUT:
      {
         ...
         if (mydevice[id].input[i].isexit && e->state)
         {
            unsigned int d,
              n;
            for (d = 0; d < MAX_DOOR; d++)
                  ...
                  if (port_device (mydoor[d].i_exit[n]) == id)
                  {
                     ...
                     if (!door_locked (d))
                     {
                        if (mydoor[d].airlock >= 0
                            && door[mydoor[d].airlock].state != DOOR_LOCKED && door[mydoor[d].airlock].state != DOOR_DEADLOCKED)
                        {
                           dolog (mydoor[d].group_lock,
                                  "DOORAIRLOCK", NULL, doorno, "Airlock violation with DOOR%02d, exit rejected", mydoor[d].airlock);
                           door_error (d);
      ...
      {                         // Check users, doors?
                     ...
                     if (door[d].state == DOOR_PROPPED || door[d].state == DOOR_OPEN || door[d].state == DOOR_PROPPEDOK)
                     {
                        ...
                        } else
                        {
                           dolog (mydoor[d].group_lock, "DOORSTILLPROPPED",
                                  u->name, doorno, "Door prop not authorised by fob %lu as not allowed", e->fob);
                           door_error (d);
                        }
                        ...
                        if (u->group_open & mydoor[d].group_lock)
                        {
                               ...
                               && door[mydoor[d].airlock].state != DOOR_LOCKED && door[mydoor[d].airlock].state != DOOR_DEADLOCKED)
                           {
                              dolog (mydoor[d].group_lock, "DOORAIRLOCK",
                                     u->name, doorno, "Airlock violation with DOOR%02d using fob %lu", mydoor[d].airlock, e->fob);
                              door_error (d);
                           } else if (mydoor[d].lockdown && (state[mydoor[d].lockdown] & mydoor[d].group_lock))
                           {    // Door in lockdown
                              dolog (mydoor[d].group_lock, "DOORLOCKDOWN",
                                     u->name, doorno, "Lockdown violation with DOOR%02d using fob %lu", mydoor[d].airlock, e->fob);
                              door_error (d);
                           } else if (mydoor[d].group_lock & ((state[STATE_SET] | state[STATE_ARM]) & ~disarm))
                           {
                              dolog (mydoor[d].group_lock, "DOORALARMED",
                                     u->name, doorno, "Door is alarmed, not opening DOOR%02d using fob %lu", d, e->fob);
                              door_error (d);
                           } else
                  ...
                  {             // Held and we are allowed to set
                     ...
                     } else
                     {
                        dolog (mydoor[d].group_lock, "FOBHELDIGNORED",
                               u->name, doorno, "Ignored held fob %lu as no setting options", e->fob);
                        door_error (d);
                     }
                  } else
                  {
                     dolog (mydoor[d].group_lock, "FOBHELDIGNORED", u->name,
                            doorno, "Ignored held fob %lu as door cannot set alarm", e->fob);
                     door_error (d);
                  }
               }
         if (!found)
         {                      // Unassociated max reader
            dolog (groups,
                   ...
                   u ? u->name : NULL, port_name (e->port), "Ignored fob %lu as reader not linked to a door", e->fob);
         }
      }
      break;
   case EVENT_KEY:
