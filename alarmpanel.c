	...
	if (id && device[id].pad)
	  {			// Prox for keypad, so somewhat different
		...
		if (k)
		  ...
		  dolog (groups, "FOBBAD", NULL, port_name (port), "Prox not linked to keypad, fob %s%s", e->fob, secure ? " (secure)" : "");
	      }
	    return;
	  }
	if (app->keypad)
			...
			if (u->group_open[secure] & mydoor[d].group_lock)
			  {
			    if (mydoor[d].airlock >= 0 && door[mydoor[d].airlock].state != DOOR_LOCKED && door[mydoor[d].airlock].state != DOOR_DEADLOCKED)
			      {
				dolog (mydoor[d].group_lock, "DOORAIRLOCK", u->name, doorno, "Airlock violation with DOOR%02d using fob %s%s", mydoor[d].airlock, e->fob, secure ? " (secure)" : "");
				door_error (d);
			      }
			    else if (mydoor[d].lockdown && (state[mydoor[d].lockdown] & mydoor[d].group_lock))
			      {	// Door in lockdown
				dolog (mydoor[d].group_lock, "DOORLOCKDOWN", u->name, doorno, "Lockdown violation with DOOR%02d using fob %s%s", mydoor[d].airlock, e->fob, secure ? " (secure)" : "");
				door_error (d);
			      }
			    else if (mydoor[d].group_lock & ((state[STATE_SET] | state[STATE_ARM]) & ~disarm))
			      {
				dolog (mydoor[d].group_lock, "DOORALARMED", u->name, doorno, "Door is alarmed, not opening DOOR%02d using fob %s%s", d, e->fob, secure ? " (secure)" : "");
				door_error (d);
			      }
			    else
