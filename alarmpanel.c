...
struct port_app_s
{
  ...
  int a, x, y;			// Location on floor plan
...
{				// Per door settings
  ...
  int a, x, y;			// Position on floor plan
  char *t;			// icon
} mydoor[MAX_DOOR] =
{
};
		...
		if (!u)
		...
		else if (e->event == EVENT_FOB)
		  {
		    ...
		    group_t disarm = ((u->group_arm & mydoor[d].group_arm & state[STATE_ARM]) | (port_name (port),
												 u->group_disarm & mydoor[d].group_disarm & state[STATE_SET]));
			...
			if (u->group_prop & mydoor[d].group_lock)
			  ...
			  {
			    dolog (mydoor[d].group_lock, "DOORSTILLPROPPED", u->name, doorno, "Door prop not authorised by fob %s as not allowed", e->fob);
			    door_error (d);
			  }
		      }
			...
			if (u->group_open & mydoor[d].group_lock)
			  {
			    if (mydoor[d].airlock >= 0 && door[mydoor[d].airlock].state != DOOR_LOCKED && door[mydoor[d].airlock].state != DOOR_DEADLOCKED)
			      {
				dolog (mydoor[d].group_lock, "DOORAIRLOCK", u->name, doorno, "Airlock violation with DOOR%02d using fob %s", mydoor[d].airlock, e->fob);
				door_error (d);
			      }
			    else if (mydoor[d].lockdown && (state[mydoor[d].lockdown] & mydoor[d].group_lock))
			      {	// Door in lockdown
				dolog (mydoor[d].group_lock, "DOORLOCKDOWN", u->name, doorno, "Lockdown violation with DOOR%02d using fob %s", mydoor[d].airlock, e->fob);
				door_error (d);
			      }
			    else if (mydoor[d].group_lock & ((state[STATE_SET] | state[STATE_ARM]) & ~disarm))
			      {
				dolog (mydoor[d].group_lock, "DOORALARMED", u->name, doorno, "Door is alarmed, not opening DOOR%02d using fob %s", d, e->fob);
				door_error (d);
			      }
			    else
		  ...
		  {		// Held and we are allowed to set
			...
			alarm_arm (u->name, port_name (port), set, mydoor[d].time_set);
		      ...
		      {
			dolog (mydoor[d].group_lock, "FOBHELDIGNORED", u->name, doorno, "Ignored held fob %s as no setting options", e->fob);
			door_error (d);
		      }
		  }
		else
		  {
		    dolog (mydoor[d].group_lock, "FOBHELDIGNORED", u->name, doorno, "Ignored held fob %s as door cannot set alarm", e->fob);
		    door_error (d);
		  }
	      }
	if (!found)
	  {			// Unassociated max reader
	    dolog (groups, e->event == EVENT_FOB_HELD ? "FOBHELDIGNORE" : "FOBIGNORED", u ? u->name : NULL, port_name (port), "Ignored fob %s as reader not linked to a door", e->fob);
	  }
      }
      break;
    case EVENT_KEY:
