...
int port_cmp(port_p a, const char *mqtt, unsigned char bus, unsigned char id, unsigned char isinput, unsigned char port)
{                               // For port ordering
...
port_p port_new_bus(unsigned char bus, unsigned char id, unsigned char isinput, unsigned char port)
{
   if (!port)
      isinput = 0;
...
port_p port_new(const char *mqtt, unsigned char isinput, unsigned char port)
{
   if (!port)
      isinput = 0;
   pthread_mutex_lock(&portmutex);
...
port_p port_new_base(port_p parent, unsigned char isinput, unsigned char port)
{
   if (parent->mqtt)
