...
server (void *arg)
{
  ...
  while (!fail)
      ...
      if (!txp && FD_ISSET (sock, &r))
	{
	  ...
	  if (pos < 2)
	    continue;
	  int i, l = 0;		// Len is low byte first
...
static rxq_t *rxq = NULL, *rxqhead = NULL;
void
mqtt_qin (j_t * jp)
{				// Queue incoming
  j_t j = *jp;
...
const char *
slot_send (slot_t id, const char *prefix, const char *suffix, j_t * jp)
{
  char *buf = NULL;
  char *topic = NULL;
  j_t j = NULL;
