...
{                               // Main RevK task
   ...
   while (1)
   {
      ...
      uint32_t range = vl53l0x_readRangeSingleMillimeters (v);
      revk_info (TAG, "Range %d", range);
#if 0
      if (vl53l0x_i2cFail (v))
      {                         // Failed
         status (ranger_fault = "Failed");
         do
            sleep (1);
         while (!vl53l0x_init (v));
         status (ranger_fault = NULL);
...
#undef u8
      if (rangerscl && rangersda)
   {
      ...
      {
         vl53l0x_t *vl53l0x = vl53l0x_config (rangerport, port_mask (rangerscl), port_mask (rangersda), rangeraddress, 0);
         if (!vl53l0x)
