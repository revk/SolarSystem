...
char *
getaes (SQL * sqlkeyp, char *target, const char *aid, const char *fob)
{                               // Get AES key (HEX ver and AES, so AES_STRING_LEN byte buffer)
   SQL_RES *res = sql_safe_query_store_free (sqlkeyp,
                                             sql_printf
                                             ("SELECT * FROM `AES` WHERE `aid`=%#s AND `fob`=%#s ORDER BY `created` DESC LIMIT 1",
                                              aid ? : "", fob ? : ""));
   ...
   if (sql_query_free
       (sqlkeyp,
        sql_printf ("INSERT INTO `AES` SET `aid`=%#s,`fob`=%#s,`ver`=%#.2s,`key`=%#s", aid ? : "", fob ? : "", target, target + 2)))
      *target = 0;
   return target;
}
         ...
         if (areas)
            while (*areas)
            {
               if (!found++)
                  ...
                  fprintf (f, ", ");
               SQL_RES *a =
                  sql_safe_query_store_free (sqlp, sql_printf ("SELECT * FROM `area` WHERE `site`=%d AND `tag`=%#c", site, *areas));
               if (sql_fetch_row (a))
...
static const char *
settings (SQL * sqlp, SQL * sqlkeyp, SQL_RES * res, slot_t id)
{                               // Security and settings
   ...
   {                            // Security
      ...
      SQL_RES *r = sql_safe_query_store_free (sqlkeyp,
                                              sql_printf
                                              ("SELECT * FROM `AES` WHERE `aid`=%#s AND `fob`='' order BY `created` DESC LIMIT 3",
                                               aid));
...
#undef led
      ...
      SQL_RES *g = sql_safe_query_store_free (sqlp,
                                              sql_printf
                                              ("SELECT * FROM `devicegpio` LEFT JOIN `gpio` USING (`gpio`) WHERE `device`=%#s AND `pcb`=%d",
...
addsitedata (SQL * sqlp, j_t j, SQL_RES * site, const char *deviceid, const char *parentid, char outofservice)
{
   ...
   v = sql_colz (site, "iottopic");
   j_store_string (j, "iottopic", *v ? v : NULL);
   ...
   } else
   {
      ...
      {                         // Make a mesh ID
         ...
         while (tries--)
         {
            ...
            if (!sql_query_free
                (sqlp, sql_printf ("UPDATE `site` SET `meshid`=%#s WHERE `site`=%#s", smac, sql_col (site, "site"))))
            {
               v = strdupa (smac);
               break;
            }
      ...
      {                         // Make a mesh ID
         ...
         while (tries--)
         {
            ...
            if (!sql_query_free
                (sqlp, sql_printf ("UPDATE `site` SET `meshkey`=%#s WHERE `site`=%#s", skey, sql_col (site, "site"))))
            {
               v = strdupa (skey);
               break;
            }
      ...
      {                         // Make mesh passphrase
         ...
         while (tries--)
         {
            ...
            if (!sql_query_free
                (sqlp, sql_printf ("UPDATE `site` SET `meshpass`=%#s WHERE `site`=%#s", spass, sql_col (site, "site"))))
            {
               v = strdupa (spass);
               break;
            }
   ...
   if (sql_fetch_row (res))
   {
      ...
      while (sql_fetch_row (device))
      {
         ...
         sql_safe_query_free (sqlp,
                              sql_printf ("UPDATE `device` SET `offlinereport`=NOW() WHERE `device`=%#s",
                                          sql_colz (device, "device")));
      }
      sql_free_result (device);
      if (l)
      {
...
const char *
doapi (SQL * sqlp, SQL * sqlkeyp, slot_t local, j_t meta, j_t j)
{
   sqlkeyp = sqlkeyp;
   local = local;
   SQL_RES *res;
   ...
   res =
      sql_safe_query_store_free (sqlp,
                                 sql_printf ("SELECT * FROM `userorganisation` WHERE `user`=%d AND `organisation`=%d", user,
                                             organisation));
   if (!sql_fetch_row (res))
   {
      sql_free_result (res);
   ...
   if (!strcmp (api, "expires"))
   {
      ...
      if (expires)
         sql_safe_query_free (sqlp,
                              sql_printf ("UPDATE `foborganisation` SET `expires`=%#T WHERE `fob`=%#s AND `organisation`=%d",
                                          expires, fob, organisation));
      else
         sql_safe_query_free (sqlp,
                              sql_printf ("UPDATE `foborganisation` SET `expires`=NULL WHERE `fob`=%#s AND `organisation`=%d", fob,
                                          organisation));
      return NULL;
   }
   ...
   while (1)
   {
         ...
         if (now / 60 != tick || poke)
         {
            ...
            res =
               sql_safe_query_store (&sql,
                                     "SELECT `site` FROM `device` WHERE `id` IS NOT NULL AND `upgrade` IS NOT NULL AND `upgrade`<NOW() GROUP BY `site`");
            while (sql_fetch_row (res))
      ...
      {                         // Check device is secure and get device SQL
         ...
         device =
            sql_safe_query_store_free (&sql,
                                       sql_printf ("SELECT * FROM `device` WHERE `device`=%#s AND `id`=%lld", deviceid ? : secureid,
                                                   id));
      ...
      {                         // Commands sent to us from local system
         ...
         {                      // Identify the device we want to talk to...
            ...
            if ((v = j_get (meta, "site")) && atoi (v))
               ...
               res =
                  sql_safe_query_store_free (&sql,
                                             sql_printf ("SELECT `id`,`pending` AS `device` FROM `pending` WHERE `pending`=%#s",
                                                         v));
            if (res)
         ...
         {                      // JSON is rest of settings to send
            ...
            if (aid && !fail)
            {
               SQL_RES *s = sql_safe_query_store_free (&sql,
                                                       sql_printf
                                                       ("SELECT * FROM `aid` LEFT JOIN `site` USING (`site`) WHERE `aid`=%#s",
                                                        aid));
               if (sql_fetch_row (s))
               {
                  j = j_create ();
            ...
            if (!fail)
               sql_safe_query_free (&sql,
                                    sql_printf ("UPDATE `pending` SET `online`=%#T WHERE `pending`=%#s", time (0) + 60, deviceid));
            return fail;
         }
         ...
         {                      // Card printing - record allocated key
            ...
            sql_free_result (res);
            sql_safe_query_free (&sqlkey,
                                 sql_printf ("INSERT INTO `AES` SET `fob`=%#s,`ver`=%#s,`key`=%#s,`aid`=''", fob, ver, key));
      ...
      {                         // From linked
         ...
         if ((v = j_get (j, "mem")) && fob)
            sql_safe_query_free (&sql,
                                 sql_printf ("UPDATE `fob` SET `mem`=%#s WHERE `fob`=%#s AND (`mem` IS NULL OR `mem`<>%#s)", v, fob,
                                             v));
         if ((v = j_get (j, "capacity")) && fob)
            sql_safe_query_free (&sql,
                                 sql_printf
                                 ("UPDATE `fob` SET `capacity`=%#s WHERE `fob`=%#s AND (`capacity` IS NULL OR `capacity`<>%#s)", v,
                                  fob, v));
         if (j_find (meta, "provisioned") && fob && (v = j_get (j, "masterkey")))
         {
            sql_safe_query_free (&sqlkey,
                                 sql_printf ("REPLACE INTO `AES` SET `fob`=%#s,`aid`='',`ver`=%#.2s,`key`=%#s", fob, v, v + 2));
            if (aid && (v = j_get (j, "aid0key")))
               sql_safe_query_free (&sqlkey,
                                    sql_printf ("REPLACE INTO `AES` SET `fob`=%#s,`aid`=%#s,`ver`=%#.2s,`key`=%#s", fob, aid, v,
                                                v + 2));
         }
         if (j_find (meta, "adopted") && fob && aid)
         {
            ...
            sql_safe_query_free (&sql,
                                 sql_printf
                                 ("INSERT IGNORE INTO `fobaid` SET `fob`=%#s,`aid`=%#s,`adopted`=NOW(),`access`=%d ON DUPLICATE KEY UPDATE `access`=%d",
                                  fob, aid, access, access));
         }
      ...
      const char *process (void)
      {
                  ...
                  if (*p)
                  {
                     ...
                     if (p - dev == 12)
                     {
                        if (secureid)
                        {
                           int site = 0;
                           SQL_RES *res =
                              sql_safe_query_store_free (&sql, sql_printf ("SELECT * FROM `device` WHERE `device`=%#s", secureid));
                           if (!sql_fetch_row (res))
                              sql_safe_query_free (&sql,
                                                   sql_printf ("INSERT INTO `device` SET `device`=%#s,`id`=%lld,`online`=NOW()",
                                                               secureid, id));
                           else
                           ...
                           device =
                              sql_safe_query_store_free (&sql,
                                                         sql_printf ("SELECT * FROM `device` WHERE `device`=%#.*s", p - dev, dev));
                           if (sql_fetch_row (device))
                           {
                              if (!strncmp (secureid, dev, p - dev))
                  ...
                  if (*p)
                  {
                     ...
                     if (p - dev == 12)
                     {
                        if (checkdevice ())
                           ...
                           sql_safe_query_free (&sql,
                                                sql_printf ("DELETE FROM `pending` WHERE `pending`=%#.*s AND `id`=%lld", p - dev,
                                                            dev, id));
                     }
                  }
               }
         ...
         {
            SQL_RES *res =
               sql_safe_query_store_free (&sql, sql_printf ("SELECT * FROM `site` WHERE `site`=%#s", sql_col (device, "site")));
            if (sql_fetch_row (res))
            {
               sql_string_t s = { };
...
#include "ESP32/main/states.m"
         ...
         if (prefix && !strcmp (prefix, "event"))
         {
            ...
            sql_safe_query_free (&sql,
                                 sql_printf
                                 ("INSERT INTO `event` SET `logged`=NOW(),`device`=%#s,`fob`=%#s,`suffix`=%#s,`data`=%#s", deviceid,
                                  fob, suffix, data));
            free (data);
            ...
            {                   // Fob usage - loads of options
               ...
               {                // Initial fob use
                  ...
                  {             // Confirm blocked
                     ...
                     {
                        sql_safe_query_free (&sql,
                                             sql_printf ("UPDATE `device` SET `poke`=NOW() WHERE `organisation`=%d", organisation));
                        poke = 1;
                     }
                  }
                  if (secure)
         ...
         } else if (prefix && !strcmp (prefix, "info"))
         {
            if (suffix && !strcmp (suffix, "upgrade") && checkdevice ())
            {
               ...
               {                // Done
                  ...
                  sql_safe_query_free (&sql,
                                       sql_printf ("UPDATE `device` SET `progress`=%d WHERE `device`=%#s",
