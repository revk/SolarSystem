...
{				// Check offline sites
  char *t;
  SQL_RES *res;
  res = sql_safe_query_store (sqlp, "SELECT `site`,SUM(if(`online` IS NOT NULL AND `outofservice`='false',1,0)) AS `N` FROM `site` LEFT JOIN `device` USING (`site`) GROUP BY `site` HAVING `N`=0");
  while (sql_fetch_row (res))
    {
