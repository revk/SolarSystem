...
const char *cakey = "", *cacert = "";
const char *mqttkey = "", *mqttcert = "";
...
static void
addarea (j_t j, const char *tag, const char *val, char always)
{
  if (!always && (!val || !*val))
    return;
  if (!val)
    val = "";
  char v[sizeof (area_t) * 8 + 1], *p = v, *e = v + sizeof (v) - 1;
  while (*val && p < e)
    {
      if (*val != ',')
	*p++ = *val;
...
void
doupgrade (SQL * sqlp, SQL_RES * site)
{				// Poking upgrades that may need doing - pick one per site, as site is one at a time
  SQL_RES *device = sql_safe_query_store_free (sqlp, sql_printf ("SELECT * FROM `device` WHERE `site`=%#s AND `upgrade`<=NOW() AND `id` IS NOT NULL ORDER BY if(`outofservice`='true',0,if(`via` IS NOT NULL,1,2)),`upgrade` LIMIT 1", sql_col (site, "site")));
...
void
dooffline (SQL * sqlp, SQL_RES * site)
{				// Check offline sites
  char *msg = NULL;
  size_t len = 0;
  FILE *f = NULL;
