...
unsigned char
key_type (const unsigned char *key)
{
   if (!key || !*key)
      return 0;
   return *key;
...
unsigned char
key_ver (const unsigned char *key)
{
   if (!key || !*key)
      return 0;
   return key[1];
...
const unsigned char *
key_aes_m (const unsigned char *key, const char *fob, unsigned char temp[16])
{
   if (!key || !*key)
      return NULL;
   if (*key == 1)
   ...
   int n;
   if (EVP_EncryptInit_ex (ctx, EVP_aes_128_cbc (), NULL, key + 2, iv) != 1 || EVP_EncryptUpdate (ctx, temp, &n, id, 16) != 1
       || EVP_EncryptFinal_ex (ctx, temp + n, &n) != 1)
   {
...
void
randkey (unsigned char key[KEY_DATA_LEN])
{
