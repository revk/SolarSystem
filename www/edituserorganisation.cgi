...
if($?userorganisationname) then
	...
	if(! $?canstrong) setenv canstrong false
	...
	if($?CANSTRONG) setenv allow "$allow canstrong"
...
<if CANSTRONG><tr><td><input type=checkbox id=canstrong name=canstrong value=true></td><td><label for=canstrong>Can strong-arm</label></td></tr></if>
