...
static void lock_open(volatile lock_t * l)
{                               // Open a lock
   if (!l->locked)
      return;
...
static void lock_lock(volatile lock_t * l)
{                               // Lock a lock
   if (l->locked)
      return;
   pthread_mutex_lock(&lockmutex);
...
static void lock_tick(volatile lock_t * l, int open)
{
   pthread_mutex_lock(&lockmutex);
   if (l->locked)
   {                            // Locked or locking
...
static void door_led(int d, unsigned char state)
{
   unsigned char led = 0;
...
void door_error(int d, const unsigned char *afile)
{                               // Indicate error
   afile = afile;
   door[d].blip = 5;
}

void door_confirm(int d, const unsigned char *afile)
...
void door_open(int d, const unsigned char *afile)
...
void door_auth(int d, const unsigned char *afile)
...
void door_lock(int d, const unsigned char *afile)
{                               // Lock
   if (door[d].autonomous)
   {
...
void door_deadlock(int d, const unsigned char *afile)
{                               // Lock and deadlock
   if (door[d].autonomous)
   {
...
void door_undeadlock(int d, const unsigned char *afile)
{                               // Un deadlock
   if (door[d].autonomous)
   {
...
void door_unlock(int d, const unsigned char *afile)
{                               // Unlock
   if (door[d].autonomous)
   {
      mqtt_door(d, "lock", afile);
